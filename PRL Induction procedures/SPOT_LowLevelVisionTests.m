...
% The point of this script is to measure visual acuity and contraast
% sensitivity.
% When measuring acuity, set the stimulus size to something around 40, so
% that the stimuli can get large, but not larger than the screen.
% When measuring contrast, set the stimulus size to  1.5*VAsize from the
% output of previously running the script for measuring acuity. (do not go below 2, which is a
% good number for healthy controls; 20 might be reasonable for MD)
% To look at the data over time, use variable Threshlist e.g.
% plot(squeeze(Threshlist))
%
...
% the same lines.
% edited by kristina and elliot on Nov 6, 2024
% goals are: (1)  to make changes to the stimulus sizes, so that it starts in the
% middle-ish of the range, and can go both up and down. -- this is
% accomplished by setting stimulussizedeg to about 40 (making the maximum
% image size large), setting the increment to .025 in line 122 "
%        Sizelist=log_unit_down(StartSize, 0.025, 90);
%        thresh(1:PRLlocations, 1:ca)=20; "  %the 20th value is where we
%        start, so we can get bigger (toward smaller indices) if needed
% 
% (2) what variable should be intput to the gui such that the contrast
% stimuli are the correct size -- VAsize*4 for MD participants (as long as the number is less than 40), 
% and a value of 2 for healthy controls 
% (3) add MD calibration to this script.
...
try
    prompt={'Participant name', 'day','scotoma active (should be 0)', 'demo (0) or session (1)',  'eye? left(1) or right(2)', 'Calibration? yes (1), no(0)', 'Eyetracker(1) or mouse(0)?', 'Calibration type: 0=regular, 1=shorter calibration with visual aids (fMD )', 'which task? VA (1) or Contrast (2)', 'stimulus size (starting value for VA)'};
    %stimulus size is in units of arcminutes of the gap in the landolt C
    %for the assumed threshold minimum angle of resolution for a standard
    %participant.
    % The gap in the C is 1/5 of the diameter of the C.
    
    name= 'Parameters';
    numlines=1;
    defaultanswer={'ftest','1', '0', '1','2','0', '0', '0', '1', '40' };  %last value was 2, for MD participants should be around 40
...
%     end
    ...
    %%this section added by em on 11/7/2024 to create a folder for
    %%participant data to be stored in 
    folderchk=cd;
    ...
    if whichTask==1    % Acuity
        ca=1; %conditions; one
        StartSize= 8; %1; %starting size for VA; 45 is about the max limit for size
        StartSize= stimulussizedeg; %1; %starting size for VA
        % Sizelist=log_unit_down(StartSize, 0.1, 90);
        Sizelist=log_unit_down(StartSize, 0.025, 90);
        %thresh(1:PRLlocations, 1:ca)=3;  % for acuity in MD participants, we may want to start nearer the middle of our total -- orig was 3 
        thresh(1:PRLlocations, 1:ca)=20;  % the larger numbers are small images
...
createO
trials=50; % This is an arbitrary number!
...
%         theeccentricity_Y=randdegarray(randi(length(randdegarray)))*pix_deg;
        % just put it in the center each time
         theeccentricity_X= 0;%wRect(3)/2;  % randdegarray(randi(length(randdegarray)))*pix_deg;
         theeccentricity_Y= 0;%wRect(4)/2; % randdegarray(randi(length(randdegarray)))*pix_deg;
