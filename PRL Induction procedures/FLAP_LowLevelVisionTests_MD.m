...
% edited to add participant assignment table kmv nov 2024
close all;
clear;
clc;
commandwindow
...
try
    ...
    prompt={'Participant name (starts with participant label)', 'Assessment day', 'practice (0) or session (1)','Calibration(1), Validation (2), or nothing(0)', 'Task: acuity (1), crowding (2), exo attention (3), contrast (4)'};
       
    name= 'Parameters';
    numlines=1; 
    defaultanswer={'fmtest1','1', '1', '1', '1'};
    ...
    tt = getParticipantAssignmentTable(SUBJECT); % get all the info about this subject.  
    % this is based on the SECOND letter in the subject name. b=uab, r=ucr,
    % m=macular degeneration, n= northeastern.
    
    % load appropriate Asssignment table %kmv and pd 20241105
%     if SUBJECT(2) == 'm'
...
%     else
%         participantAssignmentTable = fullfile(cd, ['..\..\datafolder\MDParticipantAssignmentsUAB.csv']); % then it's automatically picking this one up.  for testing.
%         'WARNING: YOU ARE RUNNING A TEST PARTICIPANT, NOT FROM A TABLE OF PARTICIPANT INFORMATION'
%     end
%     
%     participantAssignmentTable = getParticipantAssignmentTable(SUBJECT);
%     
...
% ..\datafolder\FLAP_XXX\assessment
% ..\datafolder\FLAP_XXX\assessment\sess1
% ..\datafolder\FLAP_XXX\assessment\sess2
% ..\datafolder\FLAP_XXX\assessment\sess3
    ...
    if whichTask==1    % Acuity
        ca=1; %conditions; one
        Sizelist=log_unit_down(StartSize, lu_step, 90);
        thresh(1:PRLlocations, 1:ca)=StartingElement; % changed from =3 kmv nov 2024.  to StartingElement = 20 so that it can get easier 19 times.
    end
...
%     
    ...
    for trial=1:length(mixtr)
        ...
        if SUBJECT(2)=='m'%PD added this if statement 11/5/24
        imageRect_offs_dot=[imageRectDot(1)+theeccentricity_X, imageRectDot(2)-theeccentricity_Y,...
            imageRectDot(3)+theeccentricity_X, imageRectDot(4)-theeccentricity_Y];%PD changed the sign of the theeccentricity_Y variable here
        else  imageRect_offs_dot=[imageRectDot(1)+theeccentricity_X, imageRectDot(2)+theeccentricity_Y,...
...
%             if ScotomaPresent == 1
%                 fixationscriptW
%             else  % draws fixation aids even if scotoma is inactive, still works bc scotomadeg is still set em 20241112
                fixationscriptW
%             end
            ...
            elseif (eyetime2-trial_time)>=currentpostfixationblank+currentcueISI+currentcueduration && (eyetime2-trial_time)<currentpostfixationblank+currentcueduration+currentcueISI+stimulusduration && fixating>400 && stopchecking>1 && (eyetime2-pretrial_time)<=trialTimeout
                ...
                if exist('imageRect_offs')==0
                    if whichTask~=3
                        ...
                        if SUBJECT(2)=='m'%PD addedthis if statement 11/5/24
                ...
                if whichTask == 2 % show flankers for crowding
                    if exist('imageRect_offs_flank1')==0
                        ...
                        %em changed line 576 to test 11/11/2024
                        imageRect_offs_flank2 =[imageRect_offsFlankTwo(1)-eccentricity_X2, imageRect_offsFlankTwo(2)+eccentricity_Y2,...
                            imageRect_offsFlankTwo(3)-eccentricity_X2, imageRect_offsFlankTwo(4)+eccentricity_Y2];
