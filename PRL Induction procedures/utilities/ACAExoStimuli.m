...
cueoffsetpix= [ -dotecc -dotecc; dotecc -dotecc; dotecc dotecc; -dotecc dotecc]*pix_deg;
cueoffset=repmat(cueoffsetpix,4,1);
PRLlocspix=[xlocs; ylocs]*pix_deg;
PRLlocs=repmat(PRLlocspix,PRLlocations,1);
%  PRLeccentricity_X=PRLlocs(:,1);
%  PRLeccentricity_Y=PRLlocs(:,2);
PRLeccentricity=[repmat(PRLlocspix(1,:),4,1); repmat(PRLlocspix(2,:),4,1)];
