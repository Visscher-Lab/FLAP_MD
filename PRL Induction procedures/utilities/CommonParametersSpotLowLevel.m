...
% kmv added for FLAP participants
...
elseif whichMD==8
    ...
    % scotoma 7 because this is a control participant.
elseif whichMD==10  
    scotomadegx=0;    % scotoma size in deg
    scotomadegy=0;    % scotoma size in deg
    theeccentricity_X_scotoma=0;
    theeccentricity_Y_scotoma=0;
    [img, ~, alpha] = imread('Scotoma_03.png');       
elseif  whichMD==11  
    scotomadegx=12.73;    % scotoma size in deg
    scotomadegy=12.48;    % scotoma size in deg
    theeccentricity_X_scotoma=0.97*pix_deg;
    theeccentricity_Y_scotoma=-3.22*pix_deg;
    [img, ~, alpha] = imread('Scotoma_11.png');
elseif SUBJECT(1) == 'f'; % then it's a flap participant; 
    %use the below parameters, just to make other parts of the code work
    scotomadegx=17.56;    % scotoma size in deg
    scotomadegy=12.58;    % scotoma size in deg
    theeccentricity_X_scotoma=0.63*pix_deg;
    theeccentricity_Y_scotoma=-6*pix_deg;
...
if ScotomaPresent==1
 ...
 elseif ScotomaPresent==0 || ScotomaPresent==2
     fixwindow=12;  % size of fixation window in degrees (for the beginning of trial, in the IsFixating scripts)
 end
