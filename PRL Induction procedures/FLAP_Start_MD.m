...
% modifications by Kristina Visscher Nov 2024
% Notes about variables to be used for analyses:
%   EyeSummary is a struct for each trial that has information about eye
%   position
%   
close all;
clear ;
...
try
   ...
   %participantAssignmentTable = fullfile(cd, ['..\..\datafolder\ParticipantAssignmentsUAB_corr.csv']); % uncomment this if running task at UAB
    ...
    SUBJECT = answer{1,:}; %Gets Subject Name
    tt = getParticipantAssignmentTable(SUBJECT); % get all the info about this subject.  This replaces the above
    
    site= 3;  %0; 1=bits++; 2=display++
    ScotomaPresent= str2num(tt.ScotomaPresent{1,1}); % 0 = no scotoma, 1 = scotoma
    if SUBJECT(2) == 'm' % it's an MD participant
        ScotomaPresent = 1;  % all of them will have a 'scotoma' here % k kmv 20241106
    end
...
%     calibration=1;
    ...
    %% creating stimuli
    if SUBJECT(2) == 'm' %it is an MD participant
        run(['..\..\datafolder\' char(tt.CommonParameters)]); % Most MD participants will use CommonParametersFixationtraining_MD.
        % the correct file is listed in the table
    else
    ...
    for trial=1:totalmixtr
        ...
        while eyechecked<1
            ...
            if  (eyetime2-pretrial_time)>=ITI && fixating<fixTime/ifi && stopchecking>1 && (eyetime2-pretrial_time)<=trialTimeout
                ...
                if ScotomaPresent == 1
                    if SUBJECT(2) == 'm' % it's an MD participant
                        fixationscriptW %kmv changed, just shows a bigger fixation
                    else
